//****************************************************************************
//
//     Automated Driver Installer for USB devices
//     GUI Edition.
//
//     Automated Driver Installer (Graphic)                            zadig.c
//
//****************************************************************************
/*
 *   ZADIG is a Windows application which eases the installation of USB
 *   drivers. For details, please refer to the originating website at:
 *
 *        https://zadig.akeo.ie/
 */
...
//****************************************************************************
/*
 *   To compile this application you must first have several software assets 
 *   in place.
 *
 ...
 *   Importantly:
 *
 *   (1)  Before attempting to compile this source code you must edit the
 *        "config.h" in the Embedder project and compile that first.
 *   
 *        The header file contains two references to external library 
 *        binaries LIBUSB0_DIR and LIBUSBK_DIR. These string literals must 
 *        be assigned the correct path as it is found on your development
 *        system.
 *
 *   (2)  Warning in original source code: If any part of the resulting 
 *        executable name contains "setup" or "instal(l)" it will require UAC 
 *        elevation, and, when run from a MSYS shell, will produce a 
 *        "sh: Bad file number" message.
 *
 *        See the paragraph on Automatic Elevation at:
...
//****************************************************************************
/*
 *   11-01-20  Initial commit to Open Source repository on GitHub.
 */
...
// Try to locate a funnel icon
     ...
     // We need SS_CENTERIMAGE to be able to increase the control height by two and achieve pixel positioning
          hArrow = CreateWindowExA(0, "STATIC", NULL, SS_ICON | SS_NOTIFY | SS_CENTERIMAGE | SS_REALSIZEIMAGE | WS_GROUP | WS_CHILD | WS_VISIBLE,
     ...
     // Fallback to text arrow if icons can't be used, but first change the font
          ...
          ReleaseDC(NULL, hdc);
          hf = CreateFontA(lfHeight, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, SYMBOL_CHARSET, 0, 0, PROOF_QUALITY, 0, "Wingdings");
...
// Set the log level
     ...
     if ( (profile_get_string(profile, "security", "install_cert", NULL, NULL, &tmp) == 0) && (tmp != NULL) )
     {
